...
namespace LEHuDModLauncher
{
    public partial class Launcherform : MaterialForm
    {
        ...
        public UpdateChecker updater = new();
        ...
        public Launcherform()
        {
            ...
            try
            {
                ...
                if (SettingsManager.Instance.Settings.AutoUpdate)
                {
                    try
                    {
                        _ = updater.CheckForUpdateAsync(false);
                    }
                    catch (Exception ex)
                    {
...

        {
            ...
            if (SettingsManager.Instance.Settings.DarkMode) toolStripTheme.SafeSetText("Light Theme");
            else toolStripTheme.SafeSetText("Dark Theme");
        ...
        private async Task<bool> CheckModsForUpdate(bool autoupdate, bool showmessage, bool forcemelon, bool forcemod)
        {
            ...
            if (autoupdate || forcemelon)
            {
                ...
                if (Exists(gamePath + @"\modsdl_do_not_delete\Melon.zip"))
                {
                    ...
                    if (_utils.IsLocalFileUpToDate(gamePath + @"\modsdl_do_not_delete\Melon.zip", dllist.Files[0].Url))
                    {
            ...
            if (autoupdate || forcemod) // always update with autoupdate or forcemod (install)
            {
                ...
                bool skipmoddl;
                if ((Exists(gamePath + @"\modsdl_do_not_delete\LastEpoch_Hud(Keyboard).rar") &
        ...
        private void IsModInstalled()
        {
            ...
            {
                FileInfo modfileinfo = new FileInfo(SettingsManager.Instance.Settings.GameDir + @"\Mods\LastEpoch_Hud.dll");
                pictureModInstalled.Image = Properties.Resources.check_64dp_green;
                labelVersion.SafeSetText($"({modfileinfo.LastWriteTime.ToString()})");
                return;
            }
            labelVersion.SafeSetText("");
        ...
        private void ShowGameVersion()
        {
            ...
            textGameVersion.SafeSetText(gameversion);
            Showifvalidgamefolder();
        }
