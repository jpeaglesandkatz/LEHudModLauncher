...
namespace LEHuDModLauncher
{
    public partial class Launcherform : Form
    {
        ...
        public string GetVer(byte[] ggmBytes)
        {
        ...
        //{
        //    string customEngine;
        //    return Parse(s, out customEngine);
        //}
        ...
        public void ReadGameInfo()
        {
            ...
            try
            {
                string gameDataPath = @"C:\Program Files (x86)\Steam\steamapps\common\Last Epoch\Last Epoch_Data";
                string bundlePath = @"C:\Program Files (x86)\Steam\steamapps\common\Last Epoch\Last Epoch_Data\globalgamemanagers";
                BundleFileInstance bundleFile = assetsManager.LoadBundleFile(bundlePath, true);
                instance = assetsManager.LoadAssetsFileFromBundle(bundleFile, "globalgamemanagers");
                ...
                //assetsManager.LoadClassDatabase(gameDataPath);
                //if (!instance.file.Metadata.TypeTreeEnabled)
                //    assetsManager.LoadClassDatabaseFromPackage(instance.file.Metadata.UnityVersion);
                ...
                if (assetFiles.Count > 0)
                {
                    ...
                    if (playerSettings_baseField != null)
                    {
                        AssetTypeValueField bundleVersion = playerSettings_baseField.Get("bundleVersion");
                        if (bundleVersion != null)
                            textGameVersion.Text = bundleVersion.AsString;

                    }
                    else
                    {
                        MessageBox.Show("Failed to load player settings");
                    }
                }
            }
            catch (Exception ex)
