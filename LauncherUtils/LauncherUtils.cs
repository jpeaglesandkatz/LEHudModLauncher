...
namespace LauncherUtils
{
	public class Utils
	{
		...
		public bool UserHasInternet { get; internal set; }
...

		{
			try
			{
				...
				PingReply reply = ping.Send("1.1.1.1", 2000); // 3 second timeout
		...
		public async Task RunExtraction()
		{
			if (QueExtracts.Count > 0)
			{
				...
				{
					await ExtractFileLib(extractitem.FileName, extractitem.SourcePath, extractitem.DestinationPath, extractitem.DelOriginal);
				}
				QueExtracts.Clear();
			}
		}
		...
		public async Task ExtractFileLib(string filename, string? sourcepath, string? destinationpath, bool deloriginal = false)
		{
		...
		public async Task ExtractFile(string filename, string? sourcepath, string? destinationpath, bool deloriginal = false)
		{
			try
			{
				if (sourcepath != null)
				{
					...
					if (_settings?.TmpDownloadFolder != null)
					{
				...
				try
				{
					...
					if (File.Exists(quefilepath))
					{
						...
						if (!IsLocalFileUpToDate(quefilepath, remoteFileDate, remoteFileSize))
						{
							...
							await _fileDownloader.DownloadFileAsync(item.Url, item.DownloadPath, item.Filename, 5);
						}
					...
					else await _fileDownloader.DownloadFileAsync(item.Url, item.DownloadPath, item.Filename, 5);
					// Add downloads to extaction que

				}
				catch (Exception ex)
				{
					if (_settings != null) SettingsManager.Instance.Settings.ErrorCount++;
				}
		...
		private (DateTime LastModified, long ContentLength) GetRemoteFileInfo(string url)
		{
			try
			{
				using var client = new HttpClient();
